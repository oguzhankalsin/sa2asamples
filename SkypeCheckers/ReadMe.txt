================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: SkypeCheckers プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した SkypeCheckers には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには SkypeCheckers アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

SkypeCheckers.vcproj
...
SkypeCheckers.h
   ...
   CSkypeCheckersApp アプリケーション クラスを宣言します。

SkypeCheckers.cpp
   これは、アプリケーション クラス CSkypeCheckersApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\SkypeCheckers.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル SkypeCheckers.rc に含まれていま
   す。

res\SkypeCheckers.rc2
...
SkypeCheckersDoc.h, SkypeCheckersDoc.cpp - ドキュメント
   これらのファイルは CSkypeCheckersDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CSkypeCheckersDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

SkypeCheckersView.h, SkypeCheckersView.cpp - ドキュメントのビュー
   これらのファイルは CSkypeCheckersView クラスを含みます。
   CSkypeCheckersView オブジェクトは CSkypeCheckersDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   SkypeCheckers.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
